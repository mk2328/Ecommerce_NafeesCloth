...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'muskankamran3369@gmail.com';
            $mail->Password = 'whjn yils qvte fxhx';
            $mail->SMTPSecure = 'tls';
            $mail->Port = 587;

            $mail->setFrom('muskankamran3369@gmail.com', 'E-Commerce Website');
            $mail->addAddress('muskankamran3369@gmail.com', 'Shop Owner');
            $mail->isHTML(true);
            ...
            $mail2->SMTPAuth = true;
            $mail2->Username = 'muskankamran3369@gmail.com';
            $mail2->Password = 'whjn yils qvte fxhx';
            $mail2->SMTPSecure = 'tls';
            $mail2->Port = 587;

            $mail2->setFrom('muskankamran3369@gmail.com', 'E-Commerce Website');
            $mail2->addAddress($email, $first_name . ' ' . $last_name);
            $mail2->isHTML(true);
            $mail2->Subject = '🧾 Order Confirmation - Order #' . $order_id;
            $mail2->Body = $customerMessage;
            $mail2->AltBody = strip_tags($customerMessage);
            $mail2->send();

            echo json_encode(["success" => true, "order_id" => $order_id]);
            exit();
        } catch (Exception $e) {
            echo json_encode(["success" => false, "error" => "Email Error: {$mail->ErrorInfo}"]);
            exit();
        }
    } else {
